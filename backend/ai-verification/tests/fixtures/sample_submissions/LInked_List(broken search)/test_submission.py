...
def test_search_found(ll):
    assert ll.search(20) is True          # FAILS — search bug skips correctly

def test_search_not_found(ll):
    assert ll.search(99) is False         # accidentally passes due to bug
...
def test_insert_tail_empty():
    ...
    assert lst.to_list() == [7]import pytest
from submission import LinkedList
