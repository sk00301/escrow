...
app/core/config.py
──────────────────
Centralised settings object.  Every other module imports from here — never
reads os.environ directly.  Pydantic-settings validates types and provides
defaults so the app fails loudly at startup if required vars are missing.
"""
...
class Settings(BaseSettings):
    ...
    # Generic model name — used as fallback when provider-specific model is unset.

    # ── Ollama (local, free) ──────────────────────────────────────────────────
    ollama_base_url: str = "http://localhost:11434"
    ollama_model: str = "qwen2.5-coder:latest"
    # Alternatives: codellama:13b | gemma2:9b | mistral:7b

    ollama_timeout: float = 120.0
    # Seconds before an Ollama HTTP request times out.
    # Local inference can be slow — increase for larger models.
    ...
    # LLM assessment: did the submission actually meet the stated requirements?

    llm_weight_test_execution: float = 0.30
    # Pytest pass-rate contribution to the final score.

    llm_weight_code_quality: float = 0.20
    # LLM assessment of code quality, safety, and correctness.

    llm_weight_static_analysis: float = 0.10
    # pylint / flake8 score — supporting signal only, not the primary driver.

    # ── Agent Behaviour ───────────────────────────────────────────────────────
    llm_max_retries: int = 3
    # How many times to retry complete_json() when the model returns invalid JSON.

    llm_temperature: float = 0.1
    # Low temperature → more deterministic / reproducible verdicts.

    llm_context_max_chars: int = 8000
    # Prompts longer than this are truncated before being sent to the LLM.
    # Increase if your model has a large context window and you need more code.

    # ── LLM Decision Gate Thresholds ──────────────────────────────────────────
    # These mirror ambiguity_band_* above but are scoped to the LLM pipeline
    # so each endpoint can be tuned independently without breaking the other.
    verdict_approved_threshold: float = 0.75
    # score >= this  →  APPROVED  (oracle releases payment)

    verdict_rejected_threshold: float = 0.45
    # score <  this  →  REJECTED  (between thresholds → DISPUTED / jury triggered)
