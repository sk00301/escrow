...
export function VotingHistory() {
    ...
    const votedDisputes = mockDisputes.filter((d) => d.votes.some((v) => v.jurorAddress === walletAddress) &&
        d.status === "resolved");
    const getMyVote = (dispute) => dispute.votes.find((v) => v.jurorAddress === walletAddress)?.vote;
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {votedDisputes.length === 0 ? (<EmptyState icon="vote" title="No voting history" description="Your past votes and their outcomes will appear here."/>) : (<div className="space-y-4">
              {votedDisputes.map((dispute) => {
                const myVote = getMyVote(dispute);
                const correct = wasCorrect(dispute);
                return (<div key={dispute.id} className="flex items-center justify-between rounded-lg border border-border p-4">
                    <div className="flex items-start gap-4">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-full ${correct ? "bg-primary/10" : "bg-destructive/10"}`}>
                        {correct ? (<CheckCircle className="h-5 w-5 text-primary"/>) : (<XCircle className="h-5 w-5 text-destructive"/>)}
                      </div>
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <Badge variant="outline" className={`text-xs ${correct ? "border-primary/50 text-primary" : "border-destructive/50 text-destructive"}`}>
