...
import { Scale, Clock, FileText, User, AlertTriangle, CheckCircle, Eye, ThumbsUp, ThumbsDown } from "lucide-react";
...
export function OpenDisputes() {
    ...
    const myDisputes = mockDisputes.filter((d) => d.assignedJurors.includes(walletAddress || ""));
    const pendingDisputes = myDisputes.filter((d) => d.status === "active");
    const resolvedDisputes = myDisputes.filter((d) => d.status === "resolved");
    const getMilestone = (milestoneId) => mockMilestones.find((m) => m.id === milestoneId);
    ...
    const hasVoted = (dispute) => dispute.votes.some((v) => v.jurorAddress === walletAddress);
    const handleVote = async () => {
        if (!selectedDispute || !vote)
            return;
    ...
    return (<div className="space-y-6">
      ...
      <Tabs defaultValue="pending" className="w-full">
        ...
        <TabsContent value="pending" className="mt-6">
          {pendingDisputes.length === 0 ? (<EmptyState icon="vote" title="No pending disputes" description="You have no disputes requiring your vote at this time."/>) : (<div className="grid gap-4">
              {pendingDisputes.map((dispute) => {
                const milestone = getMilestone(dispute.milestoneId);
                const voted = hasVoted(dispute);
                return (<Card key={dispute.id} className={`transition-all ${voted ? "opacity-60" : "hover:border-primary/50"}`}>
                    <CardHeader>
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <CardTitle className="text-lg">{dispute.reason}</CardTitle>
                            {voted && (<Badge variant="outline" className="bg-primary/10 text-primary">
        ...
        <TabsContent value="resolved" className="mt-6">
          {resolvedDisputes.length === 0 ? (<EmptyState icon="contract" title="No resolved disputes" description="Resolved disputes will appear here."/>) : (<div className="grid gap-4">
              {resolvedDisputes.map((dispute) => (<Card key={dispute.id}>
                  <CardHeader>
                    <CardTitle className="text-lg">{dispute.reason}</CardTitle>
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {selectedDispute?.evidence.map((item, index) => (<div key={index} className="rounded-lg border border-border p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-lg border border-border p-4">
                {getMilestone(selectedDispute?.milestoneId || "")?.deliverables.map((d, i) => (<div key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
        ...
        <DialogContent>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <RadioGroup value={vote || ""} onValueChange={(v) => setVote(v)} className="mt-3 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Label htmlFor="client" className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-border bg-background p-4 hover:bg-muted peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5">
                ...
                <div>
                  ...
                  <Label htmlFor="freelancer" className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-border bg-background p-4 hover:bg-muted peer-data-[state=checked]:border-secondary peer-data-[state=checked]:bg-secondary/5">
            ...
            <div>
              ...
              <Textarea id="reasoning" placeholder="Explain your decision based on the evidence..." value={reasoning} onChange={(e) => setReasoning(e.target.value)} rows={3} className="mt-2"/>
            </div>
          </div>
          <DialogFooter>
