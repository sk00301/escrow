...
import { MOCK_CONTRACTS, MOCK_MONTHLY_EARNINGS } from "@/lib/mock-data"
import { 
  DollarSign, 
  TrendingUp, 
  Clock, 
...
export function Earnings() {
  ...
  const completedContracts = contracts.filter(
    (c) => c.freelancerAddress === walletAddress && c.status === "released"
  )
  const pendingContracts = contracts.filter(
    (c) => c.freelancerAddress === walletAddress && ["submitted", "verified"].includes(c.status)
  )
  const inProgressContracts = contracts.filter(
    (c) => c.freelancerAddress === walletAddress && ["funded"].includes(c.status)
  )
  ...
  const myTransactions = mockTransactionHistory.filter(
