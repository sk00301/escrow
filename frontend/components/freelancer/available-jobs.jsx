...
export function AvailableJobs() {
    ...
    const availableJobs = AVAILABLE_JOBS.filter((job) => job.milestoneTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
        job.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        job.skills.some((s) => s.toLowerCase().includes(searchQuery.toLowerCase())));
    const handleApply = async () => {
        if (!selectedJob || !walletAddress)
            return;
        setIsApplying(true);
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <Input placeholder="Search jobs, skills..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10"/>
        </div>
      </div>

      {availableJobs.length === 0 ? (<EmptyState icon="milestone" title="No jobs available" description="There are no open jobs matching your search criteria. Check back later!"/>) : (<div className="grid gap-4">
          {availableJobs.map((job) => (<Card key={job.id} className="transition-all hover:border-primary/50 hover:shadow-md">
              <CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {selectedJob?.skills.map((skill) => (<Badge key={skill} variant="secondary">
                    {skill}
                  </Badge>))}
              </div>
            </div>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-2">
                {selectedJob?.deliverables.map((deliverable, index) => (<li key={index} className="flex items-start gap-2 text-sm text-muted-foreground">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea id="proposal" placeholder="Describe your experience and why you're the best fit for this job..." value={proposal} onChange={(e) => setProposal(e.target.value)} rows={5} className="mt-2"/>
            </div>
