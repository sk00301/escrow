...
export function StatusBadge({ status, pulse, className }) {
    return (<span className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border', statusStyles[status], pulse && 'animate-pulse-cyan', className)}>
      {(status === 'active' || status === 'pending') && (<span className={cn('w-1.5 h-1.5 rounded-full', status === 'active' ? 'bg-[#00B4D8]' : 'bg-[#F59E0B]')}/>)}
      {statusLabels[status]}
    </span>);
}
