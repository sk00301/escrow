...
import { Vote, Users, Clock, CheckCircle, XCircle, AlertCircle, TrendingUp, FileText, ThumbsUp, ThumbsDown, ExternalLink, Plus } from "lucide-react";
...
export default function GovernancePage() {
    ...
    const getTotalVotes = (proposal) => proposal.votesFor + proposal.votesAgainst;
    const getForPercentage = (proposal) => {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-6xl px-4 pt-24 pb-8">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {isConnected && (<Button onClick={() => setShowCreateDialog(true)}>
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatsCard title="Total Proposals" value={mockGovernanceProposals.length} icon={FileText}/>
          <StatsCard title="Participation Rate" value={67} suffix="%" icon={Users} trend={{ value: 5, positive: true }}/>
          <StatsCard title="Your Voting Power" value={500} icon={TrendingUp}/>
        </div>
        ...
        <Tabs defaultValue="active" className="w-full">
          ...
          <TabsContent value="active">
            {activeProposals.length === 0 ? (<Card>
                ...
                {activeProposals.map((proposal) => (<Card key={proposal.id} className="transition-all hover:border-primary/50">
                    <CardHeader>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <div className="relative h-3 overflow-hidden rounded-full bg-destructive/20">
                            <div className="absolute left-0 top-0 h-full bg-primary transition-all" style={{ width: `${getForPercentage(proposal)}%` }}/>
                          </div>
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          {isConnected && (<Button size="sm" onClick={() => {
          ...
          <TabsContent value="passed">
            <div className="grid gap-4">
              {passedProposals.map((proposal) => (<Card key={proposal.id} className="border-primary/30 bg-primary/5">
                  <CardHeader>
          ...
          <TabsContent value="rejected">
            <div className="grid gap-4">
              {rejectedProposals.map((proposal) => (<Card key={proposal.id} className="border-destructive/30 bg-destructive/5">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2">
        ...
        <Dialog open={!!selectedProposal && !showVoteDialog} onOpenChange={() => setSelectedProposal(null)}>
          <DialogContent className="max-w-2xl">
            ...
            <DialogFooter>
              ...
              {isConnected && selectedProposal?.status === "active" && (<Button onClick={() => setShowVoteDialog(true)}>
                  <Vote className="mr-2 h-4 w-4"/>
                  Cast Vote
                </Button>)}
            </DialogFooter>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="proposal-title" placeholder="Enter proposal title" value={newProposal.title} onChange={(e) => setNewProposal({ ...newProposal, title: e.target.value })} className="mt-2"/>
              </div>
              <div>
                <Label htmlFor="proposal-description">Description</Label>
                <Textarea id="proposal-description" placeholder="Describe your proposal in detail..." value={newProposal.description} onChange={(e) => setNewProposal({ ...newProposal, description: e.target.value })} rows={4} className="mt-2"/>
              </div>
              <div>
                <Label htmlFor="proposal-category">Category</Label>
                <Input id="proposal-category" placeholder="e.g., Parameters, Features, Economics" value={newProposal.category} onChange={(e) => setNewProposal({ ...newProposal, category: e.target.value })} className="mt-2"/>
              </div>
            </div>
            <DialogFooter>
